        ...
        if packet.is_eof():
            if current_id == last_eof_id:
                ...
                if current_hash != last_hash:
                    logging.critical(f"Received different EOF hash {sender_id}-{current_id}-{last_hash} vs {current_hash}")
            ...
            if current_id == last_chunk_id:
                ...
                if current_hash != last_hash:
                    logging.critical(f"Received different chunk hash {sender_id}-{current_id}-{last_hash} vs {current_hash}")
