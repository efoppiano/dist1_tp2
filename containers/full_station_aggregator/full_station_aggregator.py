...
class FullStationAggregator(BasicAggregator):
    def __init__(self, config: Dict[str, str]):
        ...
        super().__init__(self._city_name, config["input_queue_name"], replica_id, config["side_table_queue_name"])
    ...
    def handle_eof(self, message: Eof) -> Dict[str, List[bytes]]:
        ...
        return {
            ...
            eof_distance_calc_in_queue: [Eof().encode()],
        }
    ...
    def __check_station_name_exists(self, city_name:str, station_code: int, yearid: int) -> bool:
        ...
        if city_name not in self._stations:
            logging.warning(f"City name not found for city {city_name}")
            return False
    ...
    def handle_message(self, message: bytes) -> Dict[str, List[bytes]]:
        ...
        start_station_info = self._stations[packet.city_name][(packet.start_station_code, packet.yearid)]
        end_station_info = self._stations[packet.city_name][(packet.end_station_code, packet.yearid)]

        prec_filter_in_packet = PrecFilterIn(
            packet._city_name, packet.start_date, packet.duration_sec, packet.prectot
        )

        year_filter_in_packet = YearFilterIn(
            packet._city_name, start_station_info["station_name"], packet.yearid
        )
